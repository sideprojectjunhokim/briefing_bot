...
| `archived_at` | 하루 넘게 안 읽음 → 큐에서 내림(지우진 않음, C-25. 예전엔 3일) |
| `resurfaced_at` | **아카이브 재출고(#3).** non-null이면 화면이 "N일 전 것"이라 밝힌다 |

**왜 `archived_at`이 필요한가.** 안 내리면 큐가 무한히 자란다. 그러면 이건 읽을거리가 아니라 밀린 숙제가 되고, 밀린 숙제는 안 연다. 07-30부터 수집이 9~17시 5회로 줄면서 하루 지난 소식은 어차피 소식도 아니라고 보고 TTL도 3일 → 1일로 같이 줄였다(C-25). `briefings` 행 자체나 `source_items`는 지우지 않는다 — 큐에서만 내려간다.

**빈 회차에 행을 만들지 않는다.** 예전엔 `skipped_empty`를 남겼는데, 하루 1회일 때는 "오늘 새 소식 없음" 카드가 의미였다. 매시로 바뀌었을 땐 하루 96행의 잡음이었고, 지금(하루 5회)도 마찬가지로 의미가 없어 계속 안 만든다. 없으면 없는 것이고 화면이 "안 읽은 것 0개"라고 말한다.
