...
| 페이스 | `queueCap: 5` — 안 읽은 게 5장 있으면 그 회차는 쉰다. **읽는 속도만큼 나온다** (하루 상한은 MAX_OK_PER_DAY). 07-30: 회차가 하루 24→5회로 줄면서 2는 너무 빡빡해 5로 올림(C-25, 사실상 상한 해제) |
...
- 비용: 07-30부터 시계가 9~17시 5회/일이라 하루 1회 시절의 최대 5배. 다만 **건질 게 없으면 호출 자체를 안 하므로**(신규 0개면 LLM 안 부름) 실제 호출 수는 소스가 실제로 갱신되는 빈도에 붙는다. 모듈당 5회/일 가드가 상한(C-25, 예전엔 매시 기준 24회/일).
