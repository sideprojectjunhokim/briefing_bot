...
export const LEAD_ASK = {
  ...
  steamgame: "위시리스트에 넣을 게 있나. 세일은 이게 바닥인지, 신작은 기다릴 만한지.",
  // scp·backrooms는 탐험형이라 리드 질문이 modules/lore.ts의 LoreBrief.ask에 있다.
  // 옆칸과 겹치는지 볼 때 같이 봐라: scp="왜 무서운가", backrooms="살아나갈 수 있나".
} as const;
...
export function buildPrompt(brief: LeadBrief, ctx: PromptContext): string {
  const parts = [
    ...
    "",
    // 링크 자리에 진짜 주소를 쓰게 하면 안 된다. Google 뉴스 URL은 한 개가
    // 200자를 넘고, 여덟 개면 그것만으로 출력 예산의 절반을 먹어 본문이 잘린다.
    // 번호만 받고 주소는 코드가 붙인다 — 모델이 주소를 틀릴 일도 없어진다.
    "링크 자리에는 **주소 대신 아이템의 `n` 값**을 `#3` 꼴로 적어라.",
    "예) `- [파이썬 3.14 공개](#3) — 요약` ← 3번 아이템을 가리킨다.",
    "실제 주소는 코드가 붙인다. 주소를 직접 쓰면 그 줄은 버려진다.",
    "",
    // 원문 클릭은 더 파고들 사람만 하는 것이고, 대부분은 여기서 끝난다.
    // 그러려면 요약이 "무엇에 관한 글인지"가 아니라 "그래서 뭐가 어떻게 됐는지"여야 한다.
    "**요약은 그것만 읽고 끝낼 수 있어야 한다.** 원문은 더 깊이 볼 사람만 누른다.",
    "\"~에 대해 다룬다\", \"~를 소개한다\", \"~를 살펴본다\" 같은 문장은 아무것도 안 알려준다.",
    "실제 숫자·이름·날짜·결론을 넣어라. 기사에 그게 없으면 그 항목을 빼는 게 낫다.",
    "",
    // 소스가 영어인 날이 많다(Hacker News, 영문 Google 뉴스 등). 읽는 사람은
    // 한국어로만 본다 — 제목·리드·요약 전부. 영어를 그대로 남기면 큐레이터가 아니다.
    "**언어: 출력은 전부 한국어.** 제목·리드·요약·THREAD 문장 모두.",
    "원문(제목·본문·스니펫)이 영어면 **읽고 한국어로 옮겨 요약**해라. 영어 문장을 붙여 넣지 마라.",
    "영문 제목은 한국어로 자연스럽게 옮겨 써라. 원문 표기를 괄호로 덧붙이지 마라 — 줄만 길어진다.",
    "고유명사·제품명·기술 용어는 통용 표기를 쓰고, 그 외는 한국어로 풀어라.",
    "",
    `제외: ${brief.exclude}`,
