...
function buildContext(
  ...
  const lines = [
    ...
    "",
    "── 이 앱이 도는 방식 ──",
    "- 수집은 **한국시간 9~17시 2시간마다(9·11·13·15·17시)** 돈다(GitHub Actions cron). 정시에 딱 맞지 않고 5~20분 밀린다.",
    "- **그 시간에 건질 게 없으면 아무것도 안 쌓는다. 그게 정상이고 고장이 아니다.**",
    "  뉴스가 회차마다 쏟아지지 않으므로, 새 카드가 없는 시간이 오히려 더 많다.",
    ...
    "- 하루 넘게 안 읽은 카드는 큐에서 내려간다(지워지진 않고 관심사별 지난 목록에 남는다).",
